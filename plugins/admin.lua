...
local function run(msg,matches)
    ...
    if matches[1] == "setbotphoto" then
    	...
    	return 'لطفا عکس جدید رباتو ارسال کنید'
    end
    if matches[1] == "markread" then
    	if matches[2] == "on" then
    		redis:set("bot:markread", "on")
    		return "من پیام هارو میخونم"
    	end
    	if matches[2] == "off" then
    		redis:del("bot:markread")
    		return "من پیام هارو نمیخونم"
    	end
    	return
    end
    ...
    if matches[1] == "block" then
    	if is_admin2(matches[2]) then
    		return "شما نمیتوانید مدیرهارا بلاک کنید"
    	end
    ...
    if matches[1] == "contactlist" then
      ...
      return "لیست شماره ها در پی وی ارسال شد"
    end
    if matches[1] == "delcontact" then
      del_contact("user#id"..matches[2],ok_cb,false)
      return "کاربر "..matches[2].." از لیست شماره ها حذف شد"
    end
    if matches[1] == "dialoglist" then
      get_dialog_list(get_dialog_list_callback, {target = msg.from.id})
      return "لیست شماره ها در پیوی با فرمتی خاص ارسال شد"
    end
